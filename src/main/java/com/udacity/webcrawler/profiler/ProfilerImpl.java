...
final class ProfilerImpl implements Profiler {
	...
	public <T> T wrap(Class<T> klass, T delegate) throws IllegalArgumentException {
		...
		Object proxy = Proxy.newProxyInstance(delegate.getClass().getClassLoader(), new Class<?>[] { klass }, handler);
		return klass.cast(proxy);
	}

	@Override
	public void writeData(Path path) {
		// Write the ProfilingState data to the specified file path, creating the file
