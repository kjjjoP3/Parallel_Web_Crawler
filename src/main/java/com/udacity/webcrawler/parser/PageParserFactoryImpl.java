...
/**
 * A {@link PageParserFactory} that wraps its returned instances using a
 * {@link Profiler}.
 */
final class PageParserFactoryImpl implements PageParserFactory {
	...
	@Inject
	PageParserFactoryImpl(Profiler profiler, @IgnoredWords List<Pattern> ignoredWords, @Timeout Duration timeout) {
	...
	public PageParser get(String url) {
		// Here, parse the page with the initial timeout (instead of just the time
		// remaining), to make
		// the download less likely to fail. Deadline enforcement should happen at a
		// higher level.
