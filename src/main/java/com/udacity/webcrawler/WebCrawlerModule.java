...
/**
 * Guice dependency injection module that installs all the required dependencies
 * to run the web crawler application. Callers should use it like this:
 *
 * <pre>{@code
...
public final class WebCrawlerModule extends AbstractModule {
	...
	/**
	 * Installs a web crawler that conforms to the given
	 * {@link CrawlerConfiguration}.
	 */
	...
	protected void configure() {
		// Multibinder provides a way to implement the strategy pattern through
		// dependency injection.
		Multibinder<WebCrawler> multibinder = Multibinder.newSetBinder(binder(), WebCrawler.class, Internal.class);
		...
		install(new ParserModule.Builder().setTimeout(config.getTimeout()).setIgnoredWords(config.getIgnoredWords())
				.build());
	}
	...
	@Singleton
	@Internal
	WebCrawler provideRawWebCrawler(@Internal Set<WebCrawler> implementations,
			@TargetParallelism int targetParallelism) {
		String override = config.getImplementationOverride();
		if (!override.isEmpty()) {
			return implementations.stream().filter(impl -> impl.getClass().getName().equals(override)).findFirst()
					.orElseThrow(() -> new ProvisionException("Implementation not found: " + override));
		}
		return implementations.stream().filter(impl -> targetParallelism <= impl.getMaxParallelism()).findFirst()
				.orElseThrow(() -> new ProvisionException(
						"No implementation able to handle parallelism = \"" + config.getParallelism() + "\"."));
	}
