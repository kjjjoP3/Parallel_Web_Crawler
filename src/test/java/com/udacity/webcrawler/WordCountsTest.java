...
public final class WordCountsTest {
	...
	public void testBasicOrder() {
		...
		assertWithMessage("Returned the correct number of popular words, but the wrong words or counts").that(result)
				.containsEntry("the", 2);
		assertWithMessage("Returned the correct number of popular words, but the wrong words or counts").that(result)
		...
		assertWithMessage("Returned the correct words, but they are in the wrong order").that(result.entrySet())
	...
	public void testNotEnoughWords() {
		...
		assertWithMessage("Returned the correct number of popular words, but the wrong words or counts").that(result)
				.containsEntry("fox", 1);
		assertWithMessage("Returned the correct words, but they are in the wrong order").that(result.entrySet())
				.containsExactly(Map.entry("the", 2), Map.entry("brown", 1), Map.entry("quick", 1), Map.entry("fox", 1))
				.inOrder();
	}
}
