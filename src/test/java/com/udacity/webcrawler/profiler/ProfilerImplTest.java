...
public final class ProfilerImplTest {
	...
	public void testToString() {
		...
		assertWithMessage("The proxy should delegate toString() calls to the wrapped object.").that(proxy.toString())
	...
	public void testHashCode() {
		...
		assertWithMessage("The proxy should delegate hashCode() calls to the wrapped object.").that(proxy.hashCode())
	...
	public void testEquals() {
		...
		assertWithMessage("Each call to Profiler.wrap() should create a new proxy object.").that(proxy1)
				.isNotSameInstanceAs(proxy2);
		assertWithMessage("Two proxies should be equal if their wrapped objects are equal").that(proxy1)
				.isEqualTo(proxy2);
		assertWithMessage("Two proxies should be equal if their wrapped objects are equal").that(proxy2)
				.isEqualTo(proxy1);
	}
	...
	public void testNonObjectEquals() {
		...
		assertWithMessage("Incorrect equals() method was called").that(proxy.equals("foo", "bar")).isFalse();
	...
	public void testBasicProfiling() throws Exception {
		...
		assertWithMessage("The intercepted method did not forward the return value correctly").that(proxy.profiled())
				.isEqualTo("profiled");
		Instant afterInvocation = clock.instant();
		assertWithMessage("Expected time to advance from invocation.").that(beforeInvocation)
		...
		assertWithMessage("The profile data was not written or is incorrect").that(written)
				.contains("com.udacity.webcrawler.profiler.ProfilerImplTest$ProfiledInterfaceImpl#profiled");
	...
	public void testDeclaredExceptionHandling() throws Exception {
		...
		Instant beforeInvocation = clock.instant();
		Throwable expected = assertThrows(Throwable.class,
		...
		assertWithMessage("The proxy threw a different exception than was thrown by the wrapped object").that(expected)
				.hasMessageThat().isEqualTo("expected exception");
		...
		assertWithMessage("Streams should usually be closed in the same scope where they were created")
				.that(writer.isClosed()).isFalse();
		String written = writer.toString();
		assertWithMessage("Profile data should still be recorded if an exception was thrown.").that(written)
